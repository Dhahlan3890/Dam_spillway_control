...
image = pygame.image.load('dam ob.jpg')
icon_image = pygame.image.load('logo2.png')
...
class WaterParticle(pygame.sprite.Sprite):
    ...
    def update(self):
        # Simulate water flow by updating particle position
        self.rect.y += 1
...
class watermove(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        self.image = pygame.Surface((1.5, 5))
        self.image.fill(BLUE)
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
    ...
    def update(self):
        ...
        self.rect.x = random.uniform(0,100)
        if self.rect.y > 1000:
...
for i in range(10000):
    particle = WaterParticle(i * 8, 0)
    all_sprites.add(particle)

for i in range(10000):
    particle = watermove(i * 8, 0)
    all_sprites.add(particle)
"""
...
#########################################################################################
    ...
    for i in range(8):
        ...
        height_text1 = font1.render(f"{globals()[f"box{i+1}_level"]/10:.1f}", True, (0, 0, 0))
    ...
    """
    pygame.draw.rect(screen, L1, (750/1000*WIDTH, 50, ls_width, ls_height))
    pygame.draw.rect(screen, L2, ((750+50*1)/1000*WIDTH, 50, ls_width, ls_height))
    ...
    for i in range(WIDTH//2):
        ...
        pygame.draw.rect(screen, (0,0,255,200), (200+i, 200, 5/1000*WIDTH, water_height-vibration))
        pygame.draw.rect(screen, (0,0,255,200), (water_x, water_y, water_width, water_height))
    """
